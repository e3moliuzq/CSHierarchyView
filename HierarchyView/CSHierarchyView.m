...
- (void)createTableView:(int)index {
    ...
    if (index == 0) {
        ...
        table_view = [[UITableView alloc] initWithFrame:CGRectMake(index*6, 0, self.frame.size.width-index*6, self.frame.size.height-_name_navi_view.frame.size.height) style:UITableViewStylePlain];
    }
    ...
    [table_views_array addObject:table_view];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (arr && arr.count > 0) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            ...
            frame.origin.x = (tableView.tag+1)*6-tableView.frame.size.width;
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(hierarchyView:chooseIndexs:)]) {
            ...
            [self.delegate hierarchyView:self chooseIndexs:indexs];
        }
    }
}
...
- (void)nameNaviView:(id)sender chooseIndex:(int)index {
    ...
    int count = (int)table_views_array.count;
    
    [UIView animateWithDuration:0.2 animations:^{
        for (int i=index; i<count-1; i++) {
            UITableView *table = [table_views_array objectAtIndex:i];
            CGRect frame = table.frame;
            frame.origin.x = table.tag*6;
            table.frame = frame;
        }
        ...
        if (finished) {
            for (int i=index+1; i<count; i++) {
                UITableView *table = [table_views_array lastObject];
                [table removeFromSuperview];
                [table_views_array removeLastObject];
